...
def create_model(params, time_limit=3600, mip_focus=1, mip_gap=0.3):
    m = Model("SSTPA V3")
    ...
    # R1
    m.addConstrs((x[n, f] == 0 for n in N for f in F), name="R1")
    ...
    # R3
    m.addConstrs(
        (quicksum(x[n, f] for n in N if EL[i][n] + EV[i][n] == 1) == 1
    ...
    for i in I:
        m.addConstrs(
            (quicksum(x[n, f]
                      for n in N if EL[i][n] == 1) == quicksum(y[i][s]
                                                               for s in S[i]
                                                               if L[s][f] == 1)
             for f in F),
            name="R6",
    ...
    for i in I:
        m.addConstrs(
            (quicksum(x[n, f]
                      for n in N if EV[i][n] == 1) == quicksum(y[i][s]
                                                               for s in S[i]
                                                               if L[s][f] == 0)
             for f in F),
            name="R7",
    ...
    # R8
    m.addConstrs(
        (x[n, f] == (v_m[n, i, l, f] + e_m[n, i, l, f] + a_m[n, i, l, f])
         for n in N for i in I for f in F for l in F if f > l),
        name="R8",
    )

    # R9
    m.addConstrs(
        (x[n, f] == (v_p[n, i, l, f] + e_p[n, i, l, f] + a_p[n, i, l, f])
         for n in N for i in I for f in F for l in F if f > l),
        name="R9",
    )
    ...
    # R10
    m.addConstrs(
        ((p_m[j, i, l, f] == PI[j] + quicksum(
            quicksum(R[j][n] * x[n, theta]
                     for n in N if EL[j][n] + EV[j][n] == 1)
            for theta in F if theta > 5 and theta <= l) + quicksum(
                quicksum(3 * v_m[n, i, l, theta]
                         for theta in F if theta > l and theta <= f)
                for n in N if EL[j][n] == 1) + quicksum(
                    quicksum(3 * a_m[n, i, l, theta]
                             for theta in F if theta > l and theta <= f)
                    for n in N if EV[j][n] == 1) + quicksum(
                        quicksum(e_m[n, i, l, theta]
                                 for theta in F if theta > l and theta <= f)
                        for n in N if EL[j][n] + EV[j][n] == 1)) for j in I
         for i in I for f in F for l in F),
        name="R10",
    )

    # R11
    m.addConstrs(
        ((p_p[j, i, l, f] == PI[j] + quicksum(
            quicksum(R[j][n] * x[n, theta]
                     for n in N if EL[j][n] + EV[j][n] == 1)
            for theta in F if theta > 5 and theta <= l) + quicksum(
                quicksum(3 * v_p[n, i, l, theta]
                         for theta in F if theta > l and theta <= f)
                for n in N if EL[j][n] == 1) + quicksum(
                    quicksum(3 * a_p[n, i, l, theta]
                             for theta in F if theta > l and theta <= f)
                    for n in N if EV[j][n] == 1) + quicksum(
                        quicksum(e_p[n, i, l, theta]
                                 for theta in F if theta > l and theta <= f)
                        for n in N if EL[j][n] + EV[j][n] == 1)) for j in I
         for i in I for f in F for l in F),
        name="R11",
    ...
    # R12
    m.addConstrs(
        (((M * (alfa_m[j, i, l]) >= p_m[i, i, l, F[-1]] - p_m[j, i, l, F[-1]]))
         for i in I for j in I for l in F),
        name="R12",
    )
    # R13
    m.addConstrs(
        (((M - M * alfa_m[j, i, l] >=
           p_m[j, i, l, F[-1]] - p_m[i, i, l, F[-1]])) for i in I for j in I
         for l in F),
        name="R13",
    )
    ...
    # R14
    m.addConstrs(
        (((M * (alfa_p[j, i, l]) >= p_p[i, i, l, F[-1]] - p_p[j, i, l, F[-1]]))
         for i in I for j in I for l in F),
        name="R14",
    )
    # R15
    m.addConstrs(
        (((M - M * alfa_p[j, i, l] >=
           p_p[j, i, l, F[-1]] - p_p[i, i, l, F[-1]])) for i in I for j in I
         for l in F),
        name="R15",
    )
    ...
    for i in I:
        m.addConstrs(
            ((beta_m[i, l] == len(I) - (quicksum(alfa_m[j, i, l]
                                                 for j in I if i != j)))
             for l in F),
            name="R16",
        )
    ...
    for i in I:
        m.addConstrs(
            ((beta_p[i, l] == 1 + (quicksum((1 - alfa_p[j, i, l])
                                            for j in I if i != j)))
             for l in F),
            name="R17",
        )
